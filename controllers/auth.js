...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!user) {
    return next(new ErrorClass('There is no user with that email', 404));
  }
...
exports.updatePassword = asyncHandler(async (req, res, next) => {
  ...
  if (!(await user.matchPassword(req.body.currentPassword))) {
    return next(new ErrorClass('Password is incorrect', 401));
  }
...
const sendTokeyRes = (user, statusCode, res) => {
  // Create token
